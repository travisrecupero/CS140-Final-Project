...
public class GUIMediator {
		 ...
		 if(model.getCurrentState() != States.PROGRAM_HALTED && model.getCurrentState() != States.NOTHING_LOADED) {
			 try {
					...
					JOptionPane.showMessageDialog(frame, 
							...
							"Run time error",
							JOptionPane.OK_OPTION);
					// copy the previous catch details but change "data" to "code"
				} catch(NullPointerException e) {
					JOptionPane.showMessageDialog(frame, 
							"NullPointerException " + model.getInstrPtr() + "\n"
							+ "Exception message: " + e.getMessage(),
							"Run time error",
							JOptionPane.OK_OPTION);
					...
					JOptionPane.showMessageDialog(frame, 
							"Program Error " + model.getInstrPtr() + "\n"
							+ "Exception message: " + e.getMessage(),
							"Run time error",
							JOptionPane.OK_OPTION);
					...
					JOptionPane.showMessageDialog(frame, 
							"Divide by zero " + model.getInstrPtr() + "\n"
							+ "Exception message: " + e.getMessage(),
							"Run time error",
							JOptionPane.OK_OPTION);
	...
	private void createAndShowGUI() {
		...
		center.add(dataViewPanel1.createDataDisplay()); //create memory display
